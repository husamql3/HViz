# Contributing to hviz

Thank you for your interest in contributing to hviz! This guide will help you understand our workflow and contribution process.
...
Before contributing to hviz, make sure you have the following installed:
...
2. Clone your fork:
   ...
   ```bash
   git clone https://github.com/husamql3/hviz.git
   cd hviz
   ```
...
- `packages/core/` - Core CLI functionality, parsers, and generators
- `packages/view/` - Interactive visualization viewer
- `www/` - Marketing website (hviz.tech)
...
### hviz-specific Examples

```text
feat(core): add PostgreSQL enum support
fix(drizzle): resolve circular dependency parsing
style(view): improve table relationship colors
enhance(prisma): support composite keys
refactor(core): simplify schema parser logic
docs(www): update getting started guide
test(typeorm): add entity relationship tests
...
- `core` - Changes to the core package (CLI, parsers, generators)
- `view` - Changes to the visualization viewer
- `www` - Changes to the website
- `prisma` - Prisma-specific functionality
- `drizzle` - Drizzle-specific functionality
- `typeorm` - TypeORM-specific functionality

---

Thank you for contributing to hviz! If you have any questions, please feel free to open an issue or reach out to the maintainers.
